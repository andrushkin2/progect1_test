...
<title>Регистрация:завершение</title>
<style type="text/css">
                          ...
                          <td width="569" valign="top" nowrap>
                            ...
                            require("config.php");
								...
								$flag=true;
									...
									{
    									if (mysql_result($phones,$i,0)==$phone){
											$flag=false;
											$isNew=false;
										}
									}
								if ($flag){
									...
									$fam1 = mysql_real_escape_string($fam);
									$name1 = iconv("UTF-8", "WINDOWS-1251", $name1);
									$fam1 = iconv("UTF-8", "WINDOWS-1251", $fam1);
									mysql_query("insert into dBase (name,fam,phone,age,acces,avalible,kode,num) values ('$name1','$fam1','$phone','$age','0','0','0','$num')");//tut
								}
								mysql_query("UPDATE dBase set date ='$date' where phone='$phone' and num='$num'");//tut
								mysql_query("UPDATE dBase set record ='1' where phone='$phone' and num='$num'");//tut
								$avalible="select * from admin where name='$num'";//tut
								$res=mysql_query($avalible);
								$row=mysql_fetch_array($res);
								$rec=$row['records'];
								$temp=$rec;
								$rec=$temp+1;
								mysql_query("UPDATE admin set records ='$rec' where name='$num'");//tut
								echo "<p align=centre style=font-size:22px>$fam $name $age года рождения успешно записан(а) на мероприятие.</p>";
								echo "<p align=centre style=font-size:22px> Спасибо! Удачных Вам тренировок!</p>";
								}
								else{
									echo "<p align=centre style=font-size:22px>$fam $name, приносим Вам свои извинения. Все места на занятие уже заняты.</p>";
									echo "<script>document.getElementById(2).style.visibility=hidden;</script>";
									echo "<p align=center><a href=//salto.extreme.by><input type=button value=Выход id=44 style=font-size:22px></a></p>";
								}
							?>
    							<p align="center"><a href="//salto.extreme.by"><input type="button" value="OK" id="2" style="font-size:22px"></a></p>
...
<?php /*?><?
    ...
    mysql_select_db('salto') or die(mysql_error());
