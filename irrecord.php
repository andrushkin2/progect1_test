                            ...
                            <?php
								...
								for($i=0;$i<mysql_num_rows($phones);$i++)
								{
									if (mysql_result($phones,$i,0)==$allPhone){
										$flag=false;
										$avalible="select * from dBase where phone='$allPhone'";
										$res=mysql_query($avalible);
										$row=mysql_fetch_array($res);
										if ($row['record']=='1'){
											mysql_query("UPDATE dBase set record ='0' where phone='$allPhone'");
											$name1 = iconv("WINDOWS-1251","UTF-8", $row[name]);
											$fam1 = iconv("WINDOWS-1251","UTF-8", $row[fam]);
											echo "<p align=center><b style=font-size:22px>$fam1 $name1 успешно отписан(а) от занятия </b></p>";
										}
										if ($row['record']=='0'){
											echo "<p align=center><b style=font-size:22px>Пользователь с данным номером не записан на занятие</b></p>";
										}
									}
								}
								if($flag==true)
									echo "<p align=center><b align=center style=font-size:22px>Пользователся с данным номером нет в базе</b></p>";
								echo "<p align=center><a align=center href=//salto.extreme.by><input type=button value=Выход style=font-size:22px></a></p>"
                            ?>
                          </td>
                          <td width="19" nowrap></td>
                          <td width="7" bgcolor="#ff0100" nowrap></td>
